...
def calculate_cost(model, input_size=(1,3,224,224),input_num=1):
    input_ = torch.randn(input_size).cuda()
    # print(input_.shape)
    if input_num == 2:
        input_2 = torch.randn(input_size).cuda()
        macs, params = profile(model, inputs=(input_, input_2))
    else:
...
# CCM
class CCM(torch.nn.Module):
    def __init__(self, dim, growth_rate=2.0):
        super().__init__()
        hidden_dim = int(dim * growth_rate)

        self.ccm = torch.nn.Sequential(
            torch.nn.Conv2d(dim, hidden_dim, 3, 1, 1),
            torch.nn.GELU(),
            torch.nn.Conv2d(hidden_dim, dim, 1, 1, 0)
        )
...
class SAFM(torch.nn.Module):
    def __init__(self, dim, n_levels=4):
        ...
        # Spatial Weighting
        self.mfr = torch.nn.ModuleList([torch.nn.Conv2d(chunk_dim, chunk_dim, 3, 1, 1, groups=chunk_dim) for i in range(self.n_levels)])

        # # Feature Aggregation
        self.aggr = torch.nn.Conv2d(dim, dim, 1, 1, 0)
