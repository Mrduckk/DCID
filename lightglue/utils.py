...
def load_image(path: Path, resize: int = None,scale: float = 1.0,isTorch: bool = True, **kwargs) -> torch.Tensor:
    image = read_image(path)
    if resize is not None or scale != 1.0:
        image, _ = resize_image(image, resize, scale, **kwargs)
    if isTorch:
